...
class Install extends Api
{
    ...
    static $buildConfigFileName = 'niceadmin.php';
    ...
    public function changePackageManager()
    {
        ...
        $newPackageManager = request()->post('manager', Config::get('niceadmin.npm_package_manager'));
        if (CommandExec::instance(false)->changeTerminalConfig()) {
            $this->success('', [
                'manager' => $newPackageManager
            ]);
        } else {
            $this->error(__('Failed to switch package manager. Please modify the configuration file manually:%s', ['根目录/config/niceadmin.php']));
        }
    }
    ...
    public function baseConfig()
    {
        ...
        // 导入安装sql
        try {
            $sql = file_get_contents(root_path() . 'app' . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'niceadmin.sql');
        ...
        // 设置新的Token随机密钥key
        $oldTokenKey        = Config::get('niceadmin.token.key');
    ...
    {
        $pm = Config::get('niceadmin.npm_package_manager');
        if ($pm == 'none') {
            return false;
        }
